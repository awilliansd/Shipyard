...
export async function terminalWsRoutes(app: FastifyInstance) {
  ...
  app.post<{ Body: { projectId: string; type?: string; cols?: number; rows?: number; taskId?: string; prompt?: string; command?: string } }>(
    '/api/terminal/sessions',
    async (request, reply) => {
      if (!isAvailable()) {
      ...
      const { projectId, type = 'shell', cols = 80, rows = 24, taskId, prompt, command } = request.body;
      const projects = await getProjects();
      const project = projects.find(p => p.id === projectId);
      if (!project) return reply.status(404).send({ error: 'Project not found' });

      const sessionId = await createSession(projectId, project.path, type, cols, rows, project.name, taskId, prompt, command);
      if (!sessionId) {
